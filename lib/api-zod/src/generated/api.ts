...
export const GetBotConfigResponse = zod.object({
  "maxEntryPriceCents": zod.number().describe('Only enter if ask price per contract is at or below this (cents)'),
  "minNetProfitCents": zod.number().describe('Minimum net profit to exit (cents, after fees)'),
  "maxNetProfitCents": zod.number().describe('Maximum net profit target (cents, after fees)'),
  "minMinutesRemaining": zod.number().describe('Skip market if this many minutes or fewer remain'),
  "feeRate": zod.number().describe('Kalshi fee rate on profit (e.g. 0.07 = 7%)'),
  "pollIntervalSecs": zod.number().describe('How often to scan markets (seconds)'),
  "marketCategories": zod.array(zod.string()).describe('Which market categories to trade - \"crypto\", \"sports\", or \"all\"'),
  "cryptoCoins": zod.array(zod.string()).describe('Which crypto coins to trade e.g. [\"BTC\",\"ETH\",\"SOL\",\"DOGE\"]'),
  "maxOpenPositions": zod.number().describe('Maximum number of concurrent open trades'),
  "balanceFloorCents": zod.number().describe('Auto-stop if account balance drops below this (cents, 0 = disabled)'),
  "dailyProfitTargetCents": zod.number().describe('Auto-stop when daily profit reaches this (cents, 0 = disabled)'),
  "dailyLossLimitCents": zod.number().describe('Auto-stop when daily loss reaches this (cents, 0 = disabled)'),
  "exitWindowMins": zod.number().describe('Place limit sell order when this many minutes remain (e.g. 7)')
})
...
export const GetMomentumBotStatusResponse = zod.object({
  ...
  "healthScore": zod.union([zod.object({
  "total": zod.number(),
...
export const SetMomentumBotAutoBody = zod.object({
  "enabled": zod.boolean(),
  "balanceFloorCents": zod.number().min(setMomentumBotAutoBodyBalanceFloorCentsMin).optional(),
  "maxSessionLossCents": zod.number().min(setMomentumBotAutoBodyMaxSessionLossCentsMin).optional(),
  "consecutiveLossLimit": zod.number().min(setMomentumBotAutoBodyConsecutiveLossLimitMin).optional(),
  "betCostCents": zod.number().min(1).optional(),
  "simulatorMode": zod.boolean().optional(),
  "priceMin": zod.number().min(1).max(setMomentumBotAutoBodyPriceMinMax).optional(),
  "priceMax": zod.number().min(1).max(setMomentumBotAutoBodyPriceMaxMax).optional(),
  ...
  "staleMs": zod.number().min(setMomentumBotAutoBodyStaleMsMin).max(setMomentumBotAutoBodyStaleMsMax).optional(),
  "tpAbsoluteCents": zod.number().min(setMomentumBotAutoBodyTpAbsoluteCentsMin).max(setMomentumBotAutoBodyTpAbsoluteCentsMax).optional(),
  "sessionProfitTargetCents": zod.number().min(setMomentumBotAutoBodySessionProfitTargetCentsMin).optional(),
...
export const GetMomentumPaperStatsResponse = zod.object({
  ...
  "timeOfDay": zod.array(zod.object({
  "label": zod.string(),
  "wins": zod.number(),
  "losses": zod.number(),
  "pnlCents": zod.number()
